...
async def dashboard(request: Request):
    ...
    try:
        ...
        user_stats = {
            ...
            'avg_daily_searches_per_user': round(float(avg_daily_searches_per_user), 2),
            'retention_rate': retention_rate,
            'most_active_users_today': most_active_users_today,
            'daily_stats_week': daily_stats_week,
            'weekly_registration_trends': weekly_registration_trends,
            'monthly_registration_trends': monthly_registration_trends
        }
...
async def get_dashboard_data(request: Request):
    ...
    try:
        ...
        return JSONResponse({
            ...
            "recent_searches": [dict(search) for search in recent_searches],
            "popular_queries": [dict(query) for query in popular_queries]
        })
...
async def search_results_page(request: Request):
    """Search results page with detailed TNVED code results"""
    username = authenticate(request)
    if not username:
        return HTMLResponse("Unauthorized", status_code=401, headers={"WWW-Authenticate": "Basic"})
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                # Get recent search results with user info
                cursor.execute("""
                ...
                cursor.execute("""
                    ...
                    FROM search_results 
                    WHERE DATE(search_timestamp) = CURRENT_DATE
                """)
                today_searches = cursor.fetchone()['today_searches']
...
async def analytics_page(request: Request):
    ...
    try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                cursor.execute("""
                    SELECT 
                        ...
                        COUNT(DISTINCT CASE WHEN u.registered_at::date = ul.created_at::date THEN u.telegram_id END) as new_users_active
                    FROM usage_logs ul
                    LEFT JOIN users u ON ul.user_id = u.telegram_id
                ...
                # Weekly activity trends for the last 8 weeks
                cursor.execute("""
                    SELECT 
                        DATE_TRUNC('week', ul.created_at) as week_start,
                        COUNT(*) as total_searches,
                        COUNT(DISTINCT ul.user_id) as active_users,
                        COUNT(DISTINCT CASE WHEN u.registered_at >= DATE_TRUNC('week', ul.created_at) AND u.registered_at < DATE_TRUNC('week', ul.created_at) + INTERVAL '1 week' THEN u.telegram_id END) as new_users_active
                    FROM usage_logs ul
                    LEFT JOIN users u ON ul.user_id = u.telegram_id
                    WHERE ul.created_at >= CURRENT_DATE - INTERVAL '8 weeks'
                    GROUP BY DATE_TRUNC('week', ul.created_at)
                    ORDER BY week_start DESC
                """)
                weekly_activity = cursor.fetchall()
                
                # Monthly activity trends for the last 6 months
                cursor.execute("""
                    SELECT 
                        DATE_TRUNC('month', ul.created_at) as month_start,
                        COUNT(*) as total_searches,
                        COUNT(DISTINCT ul.user_id) as active_users,
                        COUNT(DISTINCT CASE WHEN u.registered_at >= DATE_TRUNC('month', ul.created_at) AND u.registered_at < DATE_TRUNC('month', ul.created_at) + INTERVAL '1 month' THEN u.telegram_id END) as new_users_active
                    FROM usage_logs ul
                    LEFT JOIN users u ON ul.user_id = u.telegram_id
                    WHERE ul.created_at >= CURRENT_DATE - INTERVAL '6 months'
                ...
                # User registration trends by week (last 8 weeks)
                cursor.execute("""
                    SELECT 
                        DATE_TRUNC('week', registered_at) as week_start,
                        COUNT(*) as new_users
                    FROM users
                    WHERE registered_at >= CURRENT_DATE - INTERVAL '8 weeks'
                    GROUP BY DATE_TRUNC('week', registered_at)
                    ORDER BY week_start DESC
                """)
                weekly_registrations = cursor.fetchall()
                ...
                # Top active users by day
                cursor.execute("""
                    SELECT 
                        u.full_name,
                        u.telegram_id,
                        COUNT(ul.id) as daily_searches,
                        DATE(ul.created_at) as date
                    FROM users u
                    JOIN usage_logs ul ON u.telegram_id = ul.user_id
                    WHERE ul.created_at >= CURRENT_DATE - INTERVAL '7 days'
                    GROUP BY u.telegram_id, u.full_name, DATE(ul.created_at)
                    ORDER BY daily_searches DESC, date DESC
                    LIMIT 20
                """)
                ...
                cursor.execute("""
                    SELECT 
                        ...
                        week_2_retention,
                        week_4_retention,
                        CASE WHEN total_users > 0 THEN ROUND((week_1_retention::DECIMAL / total_users * 100), 1) ELSE 0 END as week_1_retention_rate,
                        CASE WHEN total_users > 0 THEN ROUND((week_2_retention::DECIMAL / total_users * 100), 1) ELSE 0 END as week_2_retention_rate,
                        CASE WHEN total_users > 0 THEN ROUND((week_4_retention::DECIMAL / total_users * 100), 1) ELSE 0 END as week_4_retention_rate
                    FROM (
                        SELECT 
                            DATE_TRUNC('week', u.registered_at) as registration_week,
                            COUNT(DISTINCT u.telegram_id) as total_users,
                            COUNT(DISTINCT CASE WHEN ul1.created_at >= u.registered_at + INTERVAL '7 days' AND ul1.created_at < u.registered_at + INTERVAL '14 days' THEN u.telegram_id END) as week_1_retention,
                            COUNT(DISTINCT CASE WHEN ul2.created_at >= u.registered_at + INTERVAL '14 days' AND ul2.created_at < u.registered_at + INTERVAL '21 days' THEN u.telegram_id END) as week_2_retention,
                            COUNT(DISTINCT CASE WHEN ul3.created_at >= u.registered_at + INTERVAL '28 days' AND ul3.created_at < u.registered_at + INTERVAL '35 days' THEN u.telegram_id END) as week_4_retention
                        FROM users u
                        LEFT JOIN usage_logs ul1 ON u.telegram_id = ul1.user_id
                        LEFT JOIN usage_logs ul2 ON u.telegram_id = ul2.user_id
                        LEFT JOIN usage_logs ul3 ON u.telegram_id = ul3.user_id
                        WHERE u.registered_at >= CURRENT_DATE - INTERVAL '8 weeks'
                        GROUP BY DATE_TRUNC('week', u.registered_at)
                    ) retention_data
                    ORDER BY registration_week DESC
                """)
                retention_analysis = cursor.fetchall()
                ...
                cursor.execute("""
                    ...
                    ORDER BY 
                        ...
                        END
                """)
                engagement_levels = cursor.fetchall()
                ...
                # Top queries with detailed statistics
                cursor.execute("""
                    SELECT 
                        query, 
                        COUNT(*) as total_searches,
                        ...
                        ROUND(COUNT(*)::DECIMAL / COUNT(DISTINCT user_id), 1) as avg_per_user
                    FROM usage_logs 
                    ...
                    LIMIT 15
                """)
                top_queries_detailed = cursor.fetchall()
                
                # Success rate calculation
                cursor.execute("SELECT COUNT(*) as found_count FROM usage_logs")
                found_searches = cursor.fetchone()['found_count']
                
                cursor.execute("SELECT COUNT(*) as not_found_count FROM not_found_queries")
                not_found_searches = cursor.fetchone()['not_found_count']
                
                total_all_searches = found_searches + not_found_searches
                success_rate = round((found_searches / max(total_all_searches, 1)) * 100, 1) if total_all_searches > 0 else 0
                
                # Growth rate calculation
                cursor.execute("""
                    SELECT COUNT(*) as recent_users FROM users 
                    WHERE registered_at >= CURRENT_DATE - INTERVAL '30 days'
                """)
                recent_users = cursor.fetchone()['recent_users']
                growth_rate = round((recent_users / max(total_users, 1)) * 100, 1) if total_users > 0 else 0
                ...
                # Find peak hour
                if hourly_patterns:
                    peak_hour_data = max(hourly_patterns, key=lambda x: x['searches'])
                    peak_hour = int(peak_hour_data['hour'])
                else:
                    peak_hour_data = {'hour': 14, 'searches': 0}
                    peak_hour = 14
                
                # Calculate derived metrics
                avg_queries_per_user = round(total_searches / max(total_users, 1), 1) if total_users > 0 else 0
                overall_retention_rate = round((active_users_week / max(total_users, 1)) * 100, 1) if total_users > 0 else 0
        ...
        analytics_data = {
            ...
            'top_queries_detailed': top_queries_detailed,
            
            # Chart data preparation
            'daily_chart_labels': json.dumps([item['date'].strftime('%m/%d') for item in daily_activity[-30:]]),
            'daily_searches_data': json.dumps([item['total_searches'] for item in daily_activity[-30:]]),
            'daily_users_data': json.dumps([item['active_users'] for item in daily_activity[-30:]]),
            
            'weekly_chart_labels': json.dumps([item['week_start'].strftime('%m/%d') for item in weekly_activity[-8:]]),
            'weekly_searches_data': json.dumps([item['total_searches'] for item in weekly_activity[-8:]]),
            'weekly_users_data': json.dumps([item['active_users'] for item in weekly_activity[-8:]]),
            
            'monthly_chart_labels': json.dumps([item['month_start'].strftime('%b %Y') for item in monthly_activity[-6:]]),
            'monthly_searches_data': json.dumps([item['total_searches'] for item in monthly_activity[-6:]]),
            'monthly_users_data': json.dumps([item['active_users'] for item in monthly_activity[-6:]]),
            
            'hourly_chart_labels': json.dumps([f"{int(item['hour']):02d}:00" for item in hourly_patterns]),
            'hourly_searches_data': json.dumps([item['searches'] for item in hourly_patterns]),
            
            'language_labels': json.dumps([lang['language'] for lang in language_stats[:7]]),
            'language_data': json.dumps([lang['user_count'] for lang in language_stats[:7]]),
            
            'engagement_labels': json.dumps([eng['engagement_level'] for eng in engagement_levels]),
            'engagement_data': json.dumps([eng['user_count'] for eng in engagement_levels]),
            
            # Key insights
            'insight_1': f"Peak usage at {peak_hour}:00 with {peak_hour_data['searches']} searches",
            'insight_2': f"User retention rate: {overall_retention_rate}% of users are active weekly",
            'insight_3': f"Growth: {recent_users} new users in last 30 days ({growth_rate}% growth)",
            'insight_4': f"Engagement: {avg_queries_per_user} avg searches per user with {success_rate}% success rate"
        }
...
async def startup_event():
    ...
    logger.info("🚀 Starting TNVED Bot Admin Panel...")
